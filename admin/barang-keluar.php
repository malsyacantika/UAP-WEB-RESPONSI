...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi stok
    $stok = $pdo->prepare("SELECT stok_awal FROM data_buku WHERE id = ?");
    $stok->execute([$buku_id]);
    $currentStok = $stok->fetchColumn();
    
    if ($currentStok >= $jumlah) {
        // Insert barang keluar
        $stmt = $pdo->prepare("INSERT INTO barang_keluar (buku_id, jumlah, tanggal, keterangan) VALUES (?, ?, NOW(), ?)");
        ...
        $logStmt->execute([$_SESSION['user_id'], "Barang keluar: $jumlah unit buku ID $buku_id"]);
        
        $success = "Barang keluar berhasil dicatat!";
    } else {
        $error = "Stok tidak mencukupi! Stok tersedia: $currentStok";
    }
}

// Ambil data buku untuk dropdown
$bukus = $pdo->query("SELECT id, judul, stok_awal FROM data_buku WHERE stok_awal > 0 ORDER BY judul")->fetchAll(PDO::FETCH_ASSOC);

// Ambil riwayat barang keluar
$riwayat = $pdo->query("
    SELECT bk.*, db.judul 
    FROM barang_keluar bk 
    JOIN data_buku db ON bk.buku_id = db.id 
...
<body>
    ...
    <div class="container-compact">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-5">
                <div class="card-compact">
                    <div class="p-3 border-bottom">
                        <h6 class="mb-0 fw-medium">Input Barang Keluar</h6>
                    </div>
                    <div class="p-3">
                        <form method="POST">
                            <div class="mb-3">
                                <label class="form-label form-label-compact">Pilih Buku</label>
                                <select name="buku_id" class="form-select form-control-compact" required id="bukuSelect">
                                    <option value="">-- Pilih Buku --</option>
                                    <?php foreach($bukus as $buku): ?>
                                        <option value="<?= $buku['id'] ?>" data-stok="<?= $buku['stok_awal'] ?>">
                            ...
                            <div class="mb-3">
                                <label class="form-label form-label-compact">Jumlah Keluar</label>
                                <input type="number" name="jumlah" class="form-control form-control-compact" min="1" required id="jumlahInput">
                            ...
                            <button type="submit" class="btn btn-primary-compact w-100">
                                <i class="bi bi-upload me-1"></i>Catat Barang Keluar
                            </button>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-lg-7">
                <div class="card-compact">
                    <div class="p-3 border-bottom">
                        <h6 class="mb-0 fw-medium">Riwayat Barang Keluar</h6>
                    </div>
                        ...
                        <table class="table table-compact align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-danger badge-compact"><?= $r['jumlah'] ?></span>
                                    </td>
                                    <td style="font-size: 12px;">
    ...
    <script>
        ...
        document.getElementById('bukuSelect').addEventListener('change', function() {
            ...
            if (stok) {
                stokInfo.innerHTML = `<i class="bi bi-info-circle text-primary"></i> Stok tersedia: <strong>${stok}</strong> unit`;
        ...
        document.getElementById('jumlahInput').addEventListener('input', function() {
            ...
            if (max && value > max) {
                this.setCustomValidity(`Jumlah tidak boleh melebihi stok (${max})`);
            } else {
